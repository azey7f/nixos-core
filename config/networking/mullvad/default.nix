# static load-balanced+failover mullvad module
# assumes that no non-mullvad default gateway is set
{
  config,
  azLib,
  lib,
  ...
  top = config.az.core.net;
  cfg = top.mullvad;
in {
  options.az.core.net.mullvad = with azLib.opt; {
  ...
  config = lib.mkIf cfg.enable {
    ...
    in
      {
        "${cfg.endpoints.interface}".extraRoutes =
          builtins.map (server: let
            conf = config.az.core.net.mullvad.serverConfigs.${server};
          in {
            Destination =
    ...
    az.core.net.mullvad.serverConfigs = import ./servers.nix;
  };
}
